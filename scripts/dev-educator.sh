...
if [[ "${MODE}" == "web" || "${MODE}" == "--web" ]]; then
  ...
  EXPO_PUBLIC_API_URL="${API_URL}" pnpm --filter educator-app exec expo start --web --clear
  exit 0
fi
...
EXPO_PUBLIC_API_URL="${API_URL}" pnpm --filter educator-app exec expo start --lan --clear
